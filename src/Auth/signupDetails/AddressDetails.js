...
//         if (!response.ok) {
//           throw new Error(
//             `${t('AddDetails.HTTP Error! Status:')} ${response.status}`,
//           );
//         }
...
//         } catch (jsonError) {
//           throw new Error(t('AddDetails.Invalid JSON response from API'));
//         }
...
//             marginTop: hp(3),
//           }}>
//           {t('AddDetails.Choose Your PinCode')}
//         </Text>
//         <Dropdown
//           style={[
//             styles.dropdown1,
...
//           labelField="value"
//           valueField="value"
//           placeholder={t('AddDetails.PinCode')}
//           searchPlaceholder={t('AddDetails.Search...')}
...
//                 ...prevErrors,
//                 pincode: item.value ? '' : t('AddDetails.PinCode is required'),
//               }));
//           }}
//           renderLeftIcon={() => (
//             <Image
...
//           labelField="text"
//           valueField="text"
//           placeholder={t('AddDetails.City Name')}
//           searchPlaceholder={t('AddDetails.Search...')}
//           value={city || CITY}
...
//               setErrorMessageRegister(prevErrors => ({
//                 ...prevErrors,
//                 city: item.value ? '' : t('AddDetails.PinCode is required'),
//               }));
//           }}
//           renderLeftIcon={() => (
//             <Image
...
//               setErrorMessageRegister(prevErrors => ({
//                 ...prevErrors,
//                 address: text.trim() ? '' : t('AddDetails.Address is required'),
//               }));
//           }}
//           // editable={isEditable}
//           multiline={false}
...
//           }}
//           placeholder={t('AddDetails.Enter Your Complete Address')}
...
const AddressDetails = ({pageName = 'signup'}) => {
  ...
  const validateFields = useCallback(() => {
    let errors = {};
    if (!pincode) errors.pincode = t('AddDetails.PinCode is required');
    if (!city) errors.city = t('AddDetails.City is required');
    if (!address) errors.address = t('AddDetails.Address is required');

    setErrorMessageRegister?.(errors);
    return Object.keys(errors).length === 0;
  }, [pincode, city, address, t, setErrorMessageRegister]);

  useEffect(() => {
    if (!pincode || !city || !address) {
      validateFields();
    }
  }, [pincode, city, address, validateFields]);

  const fetchPincodes = useCallback(async languageCode => {
    try {
      const response = await fetch(
        `${BASE_URL}/cities?lang_code=${languageCode}`,
      );
      const result = await response.json();
    ...
    const loadLanguagePreference = async () => {
      try {
        ...
        if (savedLangCode) {
          ...
          fetchPincodes(savedLangCode);
          fetchAllCities(savedLangCode);
        }
      } catch (error) {
  ...
  }, [fetchPincodes, fetchAllCities]);

  // Listen for language changes
  useEffect(() => {
    const handleLanguageChange = newLang => {
      setLangCode(newLang);
      fetchPincodes(newLang);
      fetchAllCities(newLang);
    };
  ...
  useEffect(() => {
    ...
    if (filtered.length > 0) {
      let selectedCity = filtered.find(c => c.value === city);
      if (!selectedCity && !isCityManuallySelected.current) return;

      if (selectedCity) {
        setStateCode?.(selectedCity.stateCode);
        setStateName?.(selectedCity.stateName);
        setDistrictCode?.(selectedCity.districtCode);
        setDistrictName?.(selectedCity.districtName);
        setCityCode?.(selectedCity.cityCode);
  ...
  const handleCityChange = item => {
    ...
    setErrorMessageRegister?.(prevErrors => ({
      ...prevErrors,
      city: item.value ? '' : t('AddDetails.City is required'),
    }));
  };
      ...
      <View style={{alignItems: 'center', width: wp(83), marginTop: hp(2)}}>
        ...
        <TextInput
          ...
          style={{
            ...
            borderRadius: wp(2),
            textAlignVertical: 'top',
            borderColor: errorMessageRegister?.address ? 'red' : '#CCCCCC',
            borderWidth: errorMessageRegister?.address ? wp(0.3) : wp(0),
          }}
          placeholder={t('AddDetails.Enter Your Complete Address')}
          placeholderTextColor={'#BFBDBE'}
