...
//         if (!response.ok) {
//           throw new Error(
//             `${t('ProfDetails.HTTP error! status:')} ${response.status}`,
//           );
//         }
...
//             color: '#000000',
//           }}>
//           {t('ProfDetails.Profession Details')}
//         </Text>
...
//             marginTop: hp(3),
//           }}>
//           {t('ProfDetails.Choose Your Profession')}
//         </Text>
...
//           labelField="text"
//           valueField="id"
//           placeholder={t('ProfDetails.Profession')}
//           searchPlaceholder={t('ProfDetails.Search...')}
...
//           }}
//           placeholder={t('ProfDetails.Describe Your Profession')}
...
const ProfessionDetails = ({pageName = 'signup'}) => {
  ...
  const fetchProfessions = useCallback(
    async languageCode => {
      ...
      try {
        const response = await fetch(
          `${BASE_URL}/professions?lang_code=${languageCode}`,
          {
            headers: {
              'Content-Type': 'application/json',
              Accept: 'application/json',
    ...
    const loadLanguagePreference = async () => {
      try {
        ...
        if (savedLangCode) {
          ...
          fetchProfessions(savedLangCode);
        }
      } catch (error) {
  ...
  const handleProfessionChange = item => {
    console.log('Selected Profession:', item);
    setSelectedProfession(item.id);
    setProfession?.(item.text);
    setProfessionId?.(item.id);
    setPROFESSION?.(item.text);
    setPROFESSIONID?.(item.id);
  };

  const handleProfessionDescChange = text => {
    setProfessionDesc?.(text);
    setPROFESSIONDESC?.(text);
  };
    ...
    <ScrollView
      ...
      <View style={{alignItems: 'center', width: wp(83), marginTop: hp(2)}}>
        ...
        <TextInput
          ...
          placeholderTextColor={'#BFBDBE'}
          value={professionDesc || PROFESSIONDESC}
          onChangeText={handleProfessionDescChange}
        />
      </View>
    </ScrollView>
  );
